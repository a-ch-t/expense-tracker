...
**Важно:** промпт изначально ссылался на `src/modules/categories/` — такого каталога нет,
модули лежат плоско в `apps/api/src/`. Путь исправлен и в промпте, и в шаблоне
`.claude/templates/feature.md`, откуда ошибка тиражировалась. Новый модуль →
