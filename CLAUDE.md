...
- **После вливания ветку удаляем** (`git branch -d feat-dashboard`) вместе с её worktree,
  если он создавался.
- **Remote у репозитория пока нет**, поэтому PR открывать негде и ветка вливается локально.
  Когда remote появится, меняется только последний шаг — вместо `git merge` ветка уходит
  в `git push -u origin <ветка>` и PR; всё остальное остаётся как есть.
